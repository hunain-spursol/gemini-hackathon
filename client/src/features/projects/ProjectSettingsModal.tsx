...
const ProjectSettingsModal: React.FC<Props> = ({ project, integrations, theme, onClose, onUpdate, onDelete }) => {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className={`w-full max-w-lg rounded-2xl shadow-2xl border transition-colors ${isDark ? 'bg-[#0f0f0f] border-[#222]' : 'bg-white border-gray-200'
        }`}>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full border rounded-xl pl-10 pr-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all ${isDark ? 'bg-[#1a1a1a] border-[#333] text-white' : 'bg-gray-50 border-gray-200 text-gray-900'
                  }`}
              />
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 gap-2 max-h-40 overflow-y-auto pr-2">
              {integrations.map(int => (
                <div
                  ...
                  className={`flex items-center gap-3 p-3 border rounded-xl cursor-pointer transition-all ${selectedIds.has(int.id)
